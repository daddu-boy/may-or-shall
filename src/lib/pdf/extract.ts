 ...
 * build, Node-safe). The upload pipeline adds OCR when pages lack text, then runs this extractor again.
 */
export async function extractPdf(data: Buffer, maxPages = 500): Promise<ExtractionResult> {
  ...
  try {
    if (doc.numPages > maxPages) throw new Error(`PDF exceeds the ${maxPages} page limit.`);
